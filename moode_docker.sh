...
sudo docker create --name debian-moode --restart always -v /sys/fs/cgroup:/sys/fs/cgroup:rw -v moode:/mnt/NAS \
--privileged -e LANG=C.UTF-8 --cap-add=NET_ADMIN --security-opt seccomp:unconfined --net host \
--cgroup-parent=docker.slice --cgroupns=isolate --tmpfs /tmp --tmpfs /run --tmpfs /run/lock \
