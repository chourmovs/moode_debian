...
sudo podman run --systemd=always -u root --name=debian-arm --network=host --device /dev/snd --arch=arm --entrypoint=/sbin/init --log-level=debug --privileged --security-opt seccomp:unconfined \
