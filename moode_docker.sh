...
docker create --name debian-moode --restart always -v /sys/fs/cgroup:/sys/fs/cgroup --cgroupns=host --tmpfs /tmp --tmpfs /run --tmpfs /run/lock --net host --privileged -e LANG=C.UTF-8 --cap-add=NET_ADMIN --security-opt seccomp:unconfined navikey/raspbian-bullseye bash
