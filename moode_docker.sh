...
sudo podman run --systemd=always -td -u root --name=debian-arm -v ~/moode/sys:/sys:rw -v ~/moode/boot:/boot:rw --network=host --device /dev/snd --arch=arm --privileged --security-opt seccomp:unconfined \
